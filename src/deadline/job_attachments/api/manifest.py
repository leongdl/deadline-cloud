...
def _manifest_upload(
    ...
    uploaded to s3://{s3_bucket_name}/{root_prefix}/Manifest/{s3_key_prefix} as per the Deadline CAS folder structure.
    manifest_file: File Path to the manifest file for upload.
    s3_bucket_name: S3 bucket name.
    s3_key_prefix: S3 prefix path to the Content Addressable Storge.
    ...
    manifest_path: str = "/".join(
        [s3_key_prefix, S3_MANIFEST_FOLDER_NAME, Path(manifest_file).name]
    )
...
def _manifest_download(
    ...
    for root in merged_manifests.keys():
        ...
        manifest_name = manifest_name.removeprefix("_")
        manifest_name = f"{manifest_name}-{root_hash}-{timestamp}.manifest"
        ...
        successful_downloads.append(
            ManifestDownload(s3_key=root, local=str(local_manifest_file_path))
        )
