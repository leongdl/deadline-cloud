...
def _manifest_download(
    ...
    for input_manifest in input_manifest_paths:
        asset_manifest: BaseAssetManifest = get_manifest_from_s3(
            manifest_key=(s3_prefix / input_manifest[0]).as_posix(),
            s3_bucket=queue_s3_settings.s3BucketName,
            session=queue_role_session,
        )
        if asset_manifest is not None:
            logger.echo(f"Downloaded input manifest for root: {input_manifest[1]}")
            add_manifest_by_root(
                manifests_by_root=manifests_by_root, root=input_manifest[1], manifest=asset_manifest
            )
