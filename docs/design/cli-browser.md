...
└─────────┘  Esc ◄── └──────────┘  Esc ◄── └──────────┘
    │                                           │  │
    │ J (job attachments)                  l    │  │ j (task attachments)
    ▼                                      ▼    │  ▼
┌──────────────────┐               ┌──────────┐│ ┌──────────────────┐
...
| J | Browse job attachments (input + output) |
...
| j | Browse task attachments (input + output) |
...
- `J` from job list → shows all job-level input + output attachments
- `j` from task list → shows task-level input + output attachments (passes step_id + task_id for scoped output manifests)
...
- `run() -> Optional[tuple[str, str]]` — `l` returns `("sessions", task_id)`, `j` returns `("attachments", task_id)`, ←/Esc returns `("back", "")`, `c` copies task ID, `n`/`p` pages, `r` refresh, `q` quit
