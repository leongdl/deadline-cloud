...
def _build_test_tree() -> TreeNode:
    ...
    file_node = TreeNode(
        name="frame001.exr", node_type=NodeType.FILE, size=1024, parent=folder
    )
...
class TestAttachmentBrowserScreenClearing:
    ...
    def test_first_render_uses_hard_clear(
        ...
        boto3_session, queue_role_session = mock_sessions
        tui = AttachmentBrowserTUI(
            "farm-1", "queue-1", "job-1", "Test Job", "SUCCEEDED",
            boto3_session, queue_role_session, mock_s3_settings,
        )
        tui.run()
